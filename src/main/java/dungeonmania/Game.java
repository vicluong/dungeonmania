...
public class Game {
    ...
    public void init(
        ...
        this.id = id;
        this.battleFacade = battleFacade == null ? new BattleFacade() : battleFacade;
        map.init();
        this.tickCount = tickCount;
        player = map.getPlayer();
        player.setInventory(inventory);
        if (inventory == null) {
            player.setInventory(new Inventory(null, null));
        }
        register(() -> player.onTick(this.tickCount), PLAYER_MOVEMENT, "potionQueue");
        this.initialTreasureCount = initialTreasureCount;
        if (this.initialTreasureCount == -1)
    ...
    public Game build(String buildable) throws InvalidActionException {
        ...
        boolean hasZombies = map.getEntities(ZombieToast.class).size() > 0;
        if (!buildables.contains(buildable) || (buildable.equals("midnight_armour") && hasZombies)) {
