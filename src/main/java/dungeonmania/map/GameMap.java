...
public class GameMap {
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        for (Entity e : getEntities(entity.getPosition())) {
            if (e != entity && entity instanceof Overlappable) {
                if (e instanceof InventoryItem && entity instanceof Player) {
                    overlapCallbacks.add(() -> ((Overlappable) entity).onOverlap(this, e));
                } else if (e instanceof Overlappable) {
    ...
    public boolean canMoveSwamp(Entity entity) {
        for (Entity e : getEntities(entity.getPosition())) {
            if (e instanceof SwampTile) {
                ((SwampTile) e).onOverlap(this, entity);
                if (((SwampTile) e).isStuck(entity)) {
                    return false;
                }
            }
        }
    ...
    public List<Entity> getEntities(Position adjacent) {
        GraphNode node = nodes.get(adjacent);
    ...
    public void onTick(int tickCount) {
        List<Mercenary> mercenaries = getEntities(Mercenary.class);
        mercenaries.stream().forEach(m -> m.update());
    }
}
